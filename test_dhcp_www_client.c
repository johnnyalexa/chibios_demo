...
static uint8_t myip[4]={192,168,0,130};
...
static uint8_t netmask[4]={25,255,0,0};
...
static WORKING_AREA(dhcp_6sec_thread_wa, 128);
static msg_t dhcp_6sec_thread(void *arg){
	while(1){
	chThdSleepSeconds(6);
	dhcp_6sec_tick();
	}
...
void start_tcp_threads(void){
 /*
  ...
  chThdCreateStatic(main_tcp_thread_wa, sizeof(main_tcp_thread_wa), NORMALPRIO, main_tcp_thread, NULL);
	chThdCreateStatic(dhcp_6sec_thread_wa, sizeof(dhcp_6sec_thread_wa), NORMALPRIO, dhcp_6sec_thread, NULL);
}
